...
/// <summary>
/// Magic Missile spell - a bolt of magical energy that unerringly strikes its target.
/// 
/// This is a basic damage spell that always hits and scales with caster level.
/// Formula: 1 + (level / 2) damage
/// 
/// Metadata and formulas loaded from content/spells/spells.json
/// </summary>
public sealed class MagicMissileSpell : ISpellHandler
{
    private readonly SpellMetadata _metadata;
    private readonly FormulaEvaluator _formulaEvaluator;

    public MagicMissileSpell(SpellMetadataRegistry registry, FormulaEvaluator formulaEvaluator)
    {
        _metadata = registry.GetBySpellType(SpellType.MagicMissile)
            ?? throw new InvalidOperationException("Magic Missile spell metadata not found in registry");
    ...
    public bool CanCast(ICombatant caster)
    {
        ...
        if (caster is PlayerState player && player.GetSpell(SpellType.MagicMissile) == 0)
        {
            return false;
        }
    ...
    /// <summary>
    /// Calculate magic missile damage using Lua formula from spells.json.
    /// Formula: "return math.floor(1 + (level / 2))"
    /// Magic Missile always hits (no miss chance).
    /// </summary>
    public int CalculateDamage(ICombatant caster, ICombatant? target = null)
    {
        if (_metadata.Mechanics?.DamageFormula == null)
        {
            throw new InvalidOperationException("Magic Missile damage formula not found in metadata");
        }
    ...
    /// Formula: "return true" (Magic Missile always hits)
    /// </summary>
    public bool RollSuccess(ICombatant caster, ICombatant? target = null)
    {
        ...
        return _formulaEvaluator.EvaluateBool(
            ...
            {
                level = caster.Level,
                spellPercent = caster is PlayerState player ? player.GetSpell(SpellType.MagicMissile) : 100
            }
        );
    }
